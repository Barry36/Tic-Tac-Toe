...
var statusContent="You Go First……";
var matrix = [
	[0,0,0],
	[0,0,0],
	[0,0,0]
...
function onDown(){
	...
	if(matrix[partX][partY]==0){
		...
		if(win(partX,partY)){
			statusContent = "You Won！Click the screen to restart";
			gameover();
			addText();
		}else if(isEnd()){
			statusContent = "DRAW~~Click the screen to restart";
			gameover();
			addText();
		}else{
			statusContent = "I am thinking……";
			addText();
			computerThink();
		}
	}
...
function computerThink(){
	...
	if(win(x,y)){
		statusContent = "You Lost！Click the screen to restart";
		gameover();
		addText();
	}else if(isEnd()){
		statusContent = "DRAW~~Click the screen to restart";
